...
class Processor:
    '''
    Module: Processor (https://github.com/SilvesterHsu/LiDAR_ground_removal/blob/master/module/ground_removal.py)
    ...
    def __call__(self, vel_msg):
        ...
        # vel_non_ground = vel_non_ground * np.array([1,1,-1])
        vel_non_ground = vel_non_ground.astype(np.float32)
    ...
    def filter_out_range(self, point5D):
        ...
        radius = np.linalg.norm(point5D[:, :2], ord=2, axis=1)
